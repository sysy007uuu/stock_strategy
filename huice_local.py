...
def trace_back(mydata, file, ifplt=True):
    ...
    stock_df = pd.read_csv('stock_3_years\\stock.csv', low_memory=False)
    count = 0  # 用来遍历交易日表
    while timestr.__le__(enddate):  # 时间从开始至结束每天计算收益率，股市不开盘的日子跳过
        ...
        for key in my_stock.keys():
            if key[7:] == 'SZA':
                ...
                key[7:] = key[0:7] + 'XSHG'
            OHLC = stock_df.loc[(stock_df['time'] == my_str) & (stock_df['code'] == stock_code)]
            OHLC = OHLC.reset_index(drop=True)

            closeval = float(OHLC.at[0, 'close'])  # 当日收盘价
            # print(closeval)
        ...
        for k in range(tmp_len):
            ...
            print(OHLC)
            # print(stock_df.loc[(stock_df['time'] == timestr) & (stock_df['code'] == k)])

            openval = float(OHLC.at[0, 'open'])  # 当日开盘价
            closeval = float(OHLC.at[0, 'close'])  # 当日收盘价
